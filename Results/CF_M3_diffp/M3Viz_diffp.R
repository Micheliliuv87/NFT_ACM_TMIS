# ---- CF M3 — Forest Plot (ATE/GATES + BLP badges + R²(τ)) ----
# Reads:
#   1) cf_M3_treatment_results_with_q.csv   (ATE/GATES by treatment; from CF pipeline)
#   2) cf_M3_BLP_per_covariate.csv          (BLP per covariate; from BLP step)  [optional]
#   3) cf_M3_R2tau_by_treatment.csv         (R2_tau per treatment)              [optional fallback]
#
# Output: cf_M3_forest.pdf + .png
...
# ---------- Encoding & font (robust) ----------
try(suppressWarnings(Sys.setlocale("LC_CTYPE", "en_US.UTF-8")), silent = TRUE)

# Try a few common sans families; use one only if it's actually present.
font_candidates <- c("Linux Libertine O","Libertine","DejaVu Sans","Noto Sans","Arial","Helvetica")
pick_font <- function(cands) {
  fam <- NA_character_
  if (requireNamespace("systemfonts", quietly = TRUE)) {
    fams <- unique(systemfonts::system_fonts()$family)
    hit  <- intersect(cands, fams)
    if (length(hit)) fam <- hit[1]
  }
  fam
}
font_family <- pick_font(font_candidates)
message("Using font family: ", ifelse(is.na(font_family), "<device default>", font_family))

# Helper to open a PDF device with safe fallbacks (and Unicode support when possible)
open_pdf <- function(file, width, height, family = NA_character_) {
  use_family <- if (!is.na(family)) family else NULL
  if (capabilities("cairo")) {
    if (!is.null(use_family)) grDevices::cairo_pdf(file, width = width, height = height, family = use_family)
    else                      grDevices::cairo_pdf(file, width = width, height = height)
  } else {
    if (!is.null(use_family)) grDevices::pdf(file, width = width, height = height, family = use_family, useDingbats = FALSE)
    else                      grDevices::pdf(file, width = width, height = height, useDingbats = FALSE)
  }
}

# ---------------- Config ----------------
infile_ate  <- "cf_M3_treatment_results_with_q.csv"
infile_blp  <- "cf_M3_BLP_per_covariate.csv"
infile_r2t  <- "cf_M3_R2tau_by_treatment.csv"

outfile_pdf <- "cf_M3_forest.pdf"
outfile_png <- "cf_M3_forest.png"
...
# Subgroup ordering (Combinations first, then Strategy). Robust to "—" or "-" with/without spaces.
subgroup_order_map <- function(s) {
  s <- as.character(s)
  out <- rep(99L, length(s))
  rx <- function(x) paste0("^Combination\\s*[—-]\\s*", x, "$")
  out[grepl(rx("Seller N High"), s)]   <- 1L
  out[grepl(rx("Seller N-1 High"), s)] <- 2L
  out[grepl(rx("Buyer N High"), s)]    <- 3L
  out[grepl(rx("Buyer N-1 High"), s)]  <- 4L
  # seller multi-party
  mask_seller <- (s == "Combination — Seller") | grepl(rx("Seller$"), s)
  out[mask_seller] <- pmin(out[mask_seller], 5L)
  # "Combination — All" (Condition B: BN-1 + SN + SN-1 TVs)
  out[s == "Combination — All"] <- pmin(out[s == "Combination — All"], 6L)
  # generic "Combination"
  out[s == "Combination"] <- 6L
  # strategies
  out[grepl("^Strategy", s)] <- 20L
  out
}
...
# -------- Filter: ONLY keep (Combination & q<0.10) OR (Strategy) --------
is_combo    <- grepl("^Combination\\b", df$subgroup)
is_strategy <- grepl("^Strategy\\b",   df$subgroup)

df <- df %>%
  filter( (is_combo & is.finite(q_value_bh) & q_value_bh < 0.10) | is_strategy )

if (!nrow(df)) stop("No rows left after filtering: keeping only Combination (q<0.10) and Strategy.")
...
if (file.exists(infile_blp)) {
  ...
  if (!("treatment" %in% names(blp_raw)) && "treat" %in% names(blp_raw)) {
    data.table::setnames(blp_raw, "treat", "treatment")
  }
  if (!("covariate" %in% names(blp_raw))) {
    if ("variable" %in% names(blp_raw)) data.table::setnames(blp_raw, "variable", "covariate")
  ...
  # Per-treatment R²(τ) from BLP (if present)
  ...
  if (length(r2cand)) {
    r2_by_treat <- blp_raw %>%
      ...
      summarise(r2_tau = suppressWarnings(max(.data[[r2cand[1]]], na.rm = TRUE)),
                .groups = "drop")
    # if all NA → max(..., na.rm=TRUE) = -Inf; coerce to NA
    r2_by_treat$r2_tau[!is.finite(r2_by_treat$r2_tau)] <- NA_real_
    ...
    if (!("r2_tau" %in% names(r2t))) {
      ...
      if (length(cand)) data.table::setnames(r2t, cand[1], "r2_tau")
    }
    if (all(c("treatment","r2_tau") %in% names(r2t))) {
      ...
      pick2 <- !is.na(idx2)
      val <- suppressWarnings(as.numeric(r2t$r2_tau[idx2[pick2]]))
      r2tau_map_pct[pick2] <- 100 * val        # ← multiply ONCE (ratio → %)
    }
  }
  ...
  if (any(is.finite(r2tau_map_pct))) {
    ...
    cat(sprintf("\nOverall %s (median [min,max]): %s%% [%s%%, %s%%]\n",
                R2TAU_LABEL, fmt_pct1(median(r2vals)), fmt_pct1(min(r2vals)), fmt_pct1(max(r2vals))))
  }
...
make_rows_for_subgroup <- function(dat_sg, sg_name, blp_badge_raw, r2tau_map_pct) {
  ...
  det[[R2TAU_COL]] <- r2tau_txt
  
  rbind(head_row, det)
}

# Build table only for subgroups that have rows after filtering
plot_table <- do.call(
  rbind,
...
# ---------------- Theme ----------------
bold_names <- sort(unique(df$subgroup)) # subgroup headers present
is_sum <- plot_table$Subgroup %in% bold_names
...
# -------- Save PDF --------
open_pdf(outfile_pdf, width = 16, height = 10, family = font_family)
plot(p)
dev.off()
pdf_path <- if (file.exists(outfile_pdf)) tryCatch(normalizePath(outfile_pdf), error = function(e) outfile_pdf) else file.path(getwd(), outfile_pdf)
...
if (!requireNamespace("ragg", quietly = TRUE)) {
  png_ok <- FALSE
  message("Package 'ragg' not available; skipping PNG export. Install it to get the PNG.")
}
if (png_ok) {
  ragg::agg_png(outfile_png, width = 16, height = 5.5, units = "in", res = 900)
  plot(p); dev.off()
  png_path <- if (file.exists(outfile_png)) tryCatch(normalizePath(outfile_png), error = function(e) outfile_png) else file.path(getwd(), outfile_png)
