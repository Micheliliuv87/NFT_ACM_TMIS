...
subgroup_order_map <- function(s) {
  ...
  out[s == "Combination — Seller" | grepl(rx("Seller$"), s)] <- pmin(out[s == "Combination — Seller" | grepl(rx("Seller$"), s)], 5L)
...
if (file.exists(infile_blp)) {
  ...
  # Per-treatment R²(τ) (if provided inside the BLP file) else fallback file
  ...
  if (length(r2cand)) {
    ...
    if (all(c("treatment","r2_tau") %in% names(r2t))) {
      ...
      pick2 <- !is.na(idx2)
      r2tau_map_pct[pick2] <- 100 * r2t$r2_tau[idx2[pick2]] * 100
    }
  }
...
ragg::agg_png(outfile_png, width = 15, height = 5, units = "in", res = 900)
plot(p); dev.off()
