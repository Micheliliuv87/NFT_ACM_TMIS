...
data.table::fwrite(treat_threshold_df, "cf_diffp_treatment_thresholds.csv")
...
if (length(all_rows)) {
  ...
  data.table::fwrite(out, "cf_pds_diffp_results_combined.csv")
  data.table::fwrite(out %>% dplyr::filter(model == "GRF"), "cf_diffp_results_GRF.csv")
  data.table::fwrite(out %>% dplyr::filter(model == "PDS"), "cf_diffp_results_PDS.csv")
  
  # CATE rows (per-unit predictions on kept samples)
  wrote_cates <- FALSE
  if (length(all_cates)) {
    data.table::fwrite(dplyr::bind_rows(all_cates), "cf_diffp_results_CATEs.csv")
    wrote_cates <- TRUE
  }
  ...
  if (length(diag_overlap_rows)) {
    data.table::fwrite(dplyr::bind_rows(diag_overlap_rows), "cf_diffp_overlap_diagnostics.csv")
    wrote_ov <- TRUE
  }
  if (length(diag_balance_rows)) {
    data.table::fwrite(dplyr::bind_rows(diag_balance_rows), "cf_diffp_balance_diagnostics.csv")
    wrote_bal <- TRUE
  }
  if (length(diag_robust_rows)) {
    data.table::fwrite(dplyr::bind_rows(diag_robust_rows),  "cf_diffp_robustness.csv")
    wrote_rob <- TRUE
  }
  ...
  if (length(diag_cate_cov_overview)) {
    ...
    data.table::fwrite(cate_over, "cf_diffp_cate_by_covariate_overview.csv")
  ...
  if (nrow(out_grf)) {
    ...
    )], "cf_diffp_treatment_results_with_q.csv")
  }
  ...
  build_dashboard <- function(res_tbl, model_label) {
    ...
    out_file <- paste0("cf_diffp_dashboard_", model_label, ".csv")
    data.table::fwrite(dash, out_file)
    
    # also emit an explicitly ordered variant (handy for plotting)
    out_file_tv <- paste0("cf_diffp_dashboard_", model_label, "_treatview.csv")
  ...
  if (!is.null(file_dash_grf) && !is.null(file_dash_pds)) {
    ...
    data.table::fwrite(comp, "cf_diffp_dashboard_compare_GRF_vs_PDS.csv")
    wrote_compare <- TRUEß
  }
  ...
  cat("\n[Saved]\n",
      ...
      "  - cf_diffp_results_GRF.csv\n",
      "  - cf_diffp_results_PDS.csv\n",
      "  - cf_diffp_treatment_thresholds.csv\n",
      if (wrote_cates)         "  - cf_diffp_results_CATEs.csv\n" else "",
      if (wrote_ov)            "  - cf_diffp_overlap_diagnostics.csv\n" else "",
      if (wrote_bal)           "  - cf_diffp_balance_diagnostics.csv\n" else "",
      if (wrote_rob)           "  - cf_diffp_robustness.csv\n" else "",
      if (wrote_cate_over)     "  - cf_diffp_cate_by_covariate_overview.csv\n" else "",
      if (wrote_cate_bins)     "  - cf_diffp_cate_by_covariate_bins.csv\n" else "",
      if (wrote_cov_smd_cate)  "  - cf_diffp_covariate_smd_and_cate.csv\n" else "",
      if (!is.null(file_dash_grf)) "  - cf_diffp_dashboard_GRF.csv\n" else "",
      if (!is.null(file_dash_pds)) "  - cf_diffp_dashboard_PDS.csv\n" else "",
      if (wrote_compare)       "  - cf_diffp_dashboard_compare_GRF_vs_PDS.csv\n" else "",
      sep = "")
} else {
...
if (length(diag_r2tau_rows)) {
  ...
  data.table::fwrite(r2tau_tab, "cf_diffp_R2tau_by_treatment.csv")
}
...
if (length(diag_blp_rows)) {
  ...
  data.table::fwrite(blp_out, "cf_diffp_BLP_per_covariate.csv")
  cat("\n[Saved]\n  - cf_diffp_BLP_per_covariate.csv\n", sep = "")
}

# ----------------------- Example subgroup readout -----------------------
keys <- dt[, .(row_id, token_id, log_buyern_total_value)]
if (file.exists("cf_diffp_results_CATEs.csv")) {
  sub <- merge(fread("cf_diffp_results_CATEs.csv"), keys, by = "row_id", all.x = TRUE)
