...
# restriction: keep only rows with Buyer N-1 == Seller N
...
data.table::fwrite(treat_threshold_df, "cf_M2_samp_treatment_thresholds.csv")
...
if (length(all_rows)) {
  ...
  data.table::fwrite(out, "cf_pds_M2_samp_results_combined.csv")
  data.table::fwrite(out %>% dplyr::filter(model == "GRF"), "cf_M2_samp_results_GRF.csv")
  data.table::fwrite(out %>% dplyr::filter(model == "PDS"), "cf_M2_samp_results_PDS.csv")
  
  # CATE rows
  wrote_cates <- FALSE
  if (length(all_cates)) {
    data.table::fwrite(dplyr::bind_rows(all_cates), "cf_M2_samp_results_CATEs.csv")
    wrote_cates <- TRUE
  }
  ...
  if (length(diag_overlap_rows)) {
    data.table::fwrite(dplyr::bind_rows(diag_overlap_rows), "cf_M2_samp_overlap_diagnostics.csv")
    wrote_ov <- TRUE
  }
  if (length(diag_balance_rows)) {
    data.table::fwrite(dplyr::bind_rows(diag_balance_rows), "cf_M2_samp_balance_diagnostics.csv")
    wrote_bal <- TRUE
  }
  if (length(diag_robust_rows)) {
    data.table::fwrite(dplyr::bind_rows(diag_robust_rows),  "cf_M2_samp_robustness.csv")
    wrote_rob <- TRUE
  }
  ...
  if (length(diag_cate_cov_overview)) {
    ...
    data.table::fwrite(cate_over, "cf_M2_samp_cate_by_covariate_overview.csv")
    ...
    if (length(diag_balance_rows)) {
      ...
      data.table::fwrite(merged, "cf_M2_samp_covariate_smd_and_cate.csv")
  ...
  build_dashboard <- function(res_tbl, model_label) {
    ...
    out_file <- paste0("cf_M2_samp_dashboard_", model_label, ".csv")
    data.table::fwrite(dash, out_file)
    out_file_tv <- paste0("cf_M2_samp_dashboard_", model_label, "_treatview.csv")
  ...
  if (!is.null(file_dash_grf) && !is.null(file_dash_pds)) {
    ...
    data.table::fwrite(comp, "cf_M2_samp_dashboard_compare_GRF_vs_PDS.csv")
    wrote_compare <- TRUE
  }
  
  cat("\n[Saved]\n",
      "  - cf_pds_M2_samp_results_combined.csv\n",
      "  - cf_M2_samp_results_GRF.csv\n",
      "  - cf_M2_samp_results_PDS.csv\n",
      "  - cf_M2_samp_treatment_thresholds.csv\n",
      if (wrote_cates)         "  - cf_M2_samp_results_CATEs.csv\n" else "",
      if (wrote_ov)            "  - cf_M2_samp_overlap_diagnostics.csv\n" else "",
      if (wrote_bal)           "  - cf_M2_samp_balance_diagnostics.csv\n" else "",
      if (wrote_rob)           "  - cf_M2_samp_robustness.csv\n" else "",
      if (wrote_cate_over)     "  - cf_M2_samp_cate_by_covariate_overview.csv\n" else "",
      if (wrote_cate_bins)     "  - cf_M2_samp_cate_by_covariate_bins.csv\n" else "",
      if (wrote_cov_smd_cate)  "  - cf_M2_samp_covariate_smd_and_cate.csv\n" else "",
      if (!is.null(file_dash_grf)) "  - cf_M2_samp_dashboard_GRF.csv\n" else "",
      if (!is.null(file_dash_pds)) "  - cf_M2_samp_dashboard_PDS.csv\n" else "",
      if (wrote_compare)       "  - cf_M2_samp_dashboard_compare_GRF_vs_PDS.csv\n" else "",
      sep = "")
} else {
...
if (length(diag_r2tau_rows)) {
  ...
  data.table::fwrite(r2tau_tab, "cf_M2_samp_R2tau_by_treatment.csv")
  cat("  - cf_M2_samp_R2tau_by_treatment.csv\n")
}
...
if (length(diag_blp_rows)) {
  ...
  data.table::fwrite(blp_out, "cf_M2_samp_BLP_per_covariate.csv")
  cat("  - cf_M2_samp_BLP_per_covariate.csv\n")
}

# ----------------------- Example subgroup readout -----------------------
keys <- dt[, .(row_id, token_id, log_buyern_total_value)]
if (file.exists("cf_M2_samp_results_CATEs.csv")) {
  sub <- merge(fread("cf_M2_samp_results_CATEs.csv"), keys, by = "row_id", all.x = TRUE)
