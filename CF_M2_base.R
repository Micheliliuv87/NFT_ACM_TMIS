...
# ---- NEW: Binary with metadata (rule + cutoff) ----
to_binary_with_meta <- function(v) {
  ...
  if (length(unique(w[!is.na(w)])) == 2) {
    return(list(w = w, rule = "median",
                cutoff_log = med,
                cutoff_raw = expm1(med),   # back to original units
                share_high = mean(w == 1, na.rm = TRUE)))
  }
  # candidate 2: 75th percentile
...
# ---- Safe calibration extractor: works across grf versions ----
...
# ---- NEW: CV R2_tau for heterogeneity (ridge if glmnet is available; fallback to lasso) ----
compute_r2tau_cv <- function(X, y, K = 5, seed = 87) {
  ...
  r2_var <- if (is.finite(vY) && vY > 0) vYhat / vY else NA_real_        # Var(ŷ)/Var(y) ≈ R2_tau
  r2_oos <- if (is.finite(vY) && vY > 0) 1 - (mse / vY) else NA_real_     # OOS R^2 for sanity
...
df_offer     <- fread("Panel_for_Model2.csv", select = c("token_id","time_of_sale","total_offers","unique_makers_count","median_offer_price", "duration_offer_days"))
...
# Drop the original gas pieces so they can't leak into X or get winsorized/logged
...
for (nm in names(treat_defs)) {
  ...
  treat_threshold_meta[[nm]] <- data.frame(
    treatment   = nm,
    split_rule  = res$rule,           # "median", "q75", or ">0"
    cutoff_log  = res$cutoff_log,     # on the log1p scale
    cutoff_raw  = res$cutoff_raw,     # back to original units
    share_high0 = res$share_high,     # full-sample share in high group
...
data.table::fwrite(treat_threshold_df, "cf_M2_V7_up_treatment_thresholds.csv")
...
# NEW: CATE vs covariate diagnostics
diag_cate_cov_overview <- list()  # per (treat × covariate): corr/slope or 1–0 diff
diag_cate_cov_bins     <- list()  # per (treat × covariate × bin): mean/sd CATE

# NEW: BLP (per-covariate) containers
diag_blp_rows <- list()

# NEW: per-treatment R2_tau
...
safe_blp_single <- function(cf, x_col_matrix) {
  # Try several signatures to be robust across grf versions (and ask for SEs if possible).
  ...
  if (!is.null(out)) return(out)
  # Without compute.se (older versions)
...
parse_blp_coef_se <- function(blp_obj) {
  ...
  # 2) Matrix / data.frame fallback (assume [1]=intercept, [2]=slope; columns are estimate, se)
  ...
  # 3) Atomic numeric vector: likely just coefficients (no SE provided).
  #    Return NULL to trigger OLS fallback for SEs.
...
run_models_for_treatment <- function(W, tname, y) {
  ...
  # Define X_kept early (used by several blocks)
  ...
  cat(sprintf("  Overlap α=%s | kept %d / %d (%.1f%%)\n",
              as.character(alpha_used), ov$n_kept, ov$n_total, ov$kept_pct))
  cat("  Propensity (all):  ", prop_str_all,  "\n", sep = "")
  cat("  Propensity (kept): ", prop_str_keep, "\n", sep = "")
  cat(sprintf("  Median |SMD| raw=%.3f → kept=%.3f; 90th |SMD| raw=%.3f → kept=%.3f\n",
              median(abs(smd_raw), na.rm=TRUE), median(abs(smd_keep), na.rm=TRUE),
              quantile(abs(smd_raw), 0.9, na.rm=TRUE), quantile(abs(smd_keep), 0.9, na.rm=TRUE)))
  ...
  # Robustness ATEs: only if propensities not extreme
  ...
  # Optional AIPW on kept (fixed indexing)
  ...
  # ----- NEW: Per-treatment heterogeneity R2_tau (CV) -----
  r2pair <- compute_r2tau_cv(X_kept, cate_log, K = 5, seed = 87)
  diag_r2tau_rows[[tname]] <<- data.frame(
    treatment   = tname,
    r2_tau      = as.numeric(r2pair["r2_tau"]),      # fraction 0–1
    r2_tau_oos  = as.numeric(r2pair["r2_tau_oos"]),  # OOS R^2
    n_kept      = n_used,
    stringsAsFactors = FALSE
  )
  ...
  # ---- NEW: CATE vs. covariate (on KEPT sample) ----
  ...
  for (j in seq_len(ncol(X_kept))) {
    ...
    # Try GRF BLP on the single covariate
    ...
    lo  <- beta - 1.96*se
    hi  <- beta + 1.96*se
    # Percent-scale interpretation (delta method): 100*(exp(beta)-1)
...
if (length(all_rows)) {
  ...
  data.table::fwrite(out, "cf_pds_M2_V7_up_results_combined.csv")
  data.table::fwrite(out %>% dplyr::filter(model == "GRF"), "cf_M2_V7_up_results_GRF.csv")
  data.table::fwrite(out %>% dplyr::filter(model == "PDS"), "cf_M2_V7_up_results_PDS.csv")
  
  # CATE rows (per-unit predictions on kept samples)
  wrote_cates <- FALSE
  if (length(all_cates)) {
    data.table::fwrite(dplyr::bind_rows(all_cates), "cf_M2_V7_up_results_CATEs.csv")
    wrote_cates <- TRUE
  }
  
  # Diagnostics tables (overlap/balance/robustness if any)
  wrote_ov <- FALSE; wrote_bal <- FALSE; wrote_rob <- FALSE
  if (length(diag_overlap_rows)) {
    data.table::fwrite(dplyr::bind_rows(diag_overlap_rows), "cf_M2_V7_up_overlap_diagnostics.csv")
    wrote_ov <- TRUE
  }
  if (length(diag_balance_rows)) {
    data.table::fwrite(dplyr::bind_rows(diag_balance_rows), "cf_M2_V7_up_balance_diagnostics.csv")
    wrote_bal <- TRUE
  }
  if (length(diag_robust_rows)) {
    data.table::fwrite(dplyr::bind_rows(diag_robust_rows),  "cf_M2_V7_up_robustness.csv")
    wrote_rob <- TRUE
  }
  ...
  if (length(diag_cate_cov_overview)) {
    ...
    data.table::fwrite(cate_over, "cf_M2_V7_up_cate_by_covariate_overview.csv")
  ...
  # Balance summary (shared across models because trimming/keep is shared)
  ...
  # Overlap (kept %) summary (safe empty if none)
  if (length(diag_overlap_rows)) {
  ...
  # ---- NEW: bring in treatment thresholds to dashboards ----
  thresholds_for_join <- treat_threshold_df %>%
    dplyr::select(treatment, split_rule, cutoff_raw)
  
  # Helper to build a dashboard per model
  # =========================
  # NEW: GATES + forest-ready summary (self-contained)
  # =========================
  ...
  # map verbose treatment → subgroup/metric/pretty label
  parse_treatment_label <- function(tnm) {
    ...
    if (m[1] < 0) return(list(
      subgroup = tnm, metric = tnm, pretty = tnm,
    ...
    metric_order <- switch(metric,
                           ...
                           "Gas Limit Cost" = 3L,
                           99L)
    list(subgroup = subgroup, metric = metric,
  ...
  # optional: paper-style table (local folder)
  if (nrow(gates_tbl)) {
  ...
  build_dashboard <- function(res_tbl, model_label) {
    ...
    out_file <- paste0("cf_M2_V7_up_dashboard_", model_label, ".csv")
    data.table::fwrite(dash, out_file)
    
    # also emit an explicitly ordered variant (handy for plotting)
    out_file_tv <- paste0("cf_M2_V7_up_dashboard_", model_label, "_treatview.csv")
    data.table::fwrite(dash, out_file_tv)
    
    out_file
  }
  
  # rebuild dashboards with GATES included
  file_dash_grf <- build_dashboard(out_grf, "GRF")
  file_dash_pds <- build_dashboard(out_pds, "PDS")
  
  
  # Optional: side-by-side GRF vs PDS comparison
  wrote_compare <- FALSE
  if (!is.null(file_dash_grf) && !is.null(file_dash_pds)) {
    ...
    data.table::fwrite(comp, "cf_M2_V7_up_dashboard_compare_GRF_vs_PDS.csv")
    wrote_compare <- TRUE
  }
  
  # Consolidated "Saved" printout (after dashboards exist)
  cat("\n[Saved]\n",
      "  - cf_pds_M2_V7_up_results_combined.csv\n",
      "  - cf_M2_V7_up_results_GRF.csv\n",
      "  - cf_M2_V7_up_results_PDS.csv\n",
      "  - cf_M2_V7_up_treatment_thresholds.csv\n",
      if (wrote_cates)         "  - cf_M2_V7_up_results_CATEs.csv\n" else "",
      if (wrote_ov)            "  - cf_M2_V7_up_overlap_diagnostics.csv\n" else "",
      if (wrote_bal)           "  - cf_M2_V7_up_balance_diagnostics.csv\n" else "",
      if (wrote_rob)           "  - cf_M2_V7_up_robustness.csv\n" else "",
      if (wrote_cate_over)     "  - cf_M2_V7_up_cate_by_covariate_overview.csv\n" else "",
      if (wrote_cate_bins)     "  - cf_M2_V7_up_cate_by_covariate_bins.csv\n" else "",
      if (wrote_cov_smd_cate)  "  - cf_M2_V7_up_covariate_smd_and_cate.csv\n" else "",
      if (!is.null(file_dash_grf)) "  - cf_M2_V7_up_dashboard_GRF.csv\n" else "",
      if (!is.null(file_dash_pds)) "  - cf_M2_V7_up_dashboard_PDS.csv\n" else "",
      if (wrote_compare)       "  - cf_M2_V7_up_dashboard_compare_GRF_vs_PDS.csv\n" else "",
      sep = "")
} else {
  cat("\n[Note] No results produced; check overlap trimming & treatment variation.\n")
}

# ----------------------- NEW: Save R2_tau (per treatment) -----------------------
if (length(diag_r2tau_rows)) {
  r2tau_tab <- dplyr::bind_rows(diag_r2tau_rows)
  data.table::fwrite(r2tau_tab, "cf_M2_V7_up_R2tau_by_treatment.csv")
}

# ----------------------- NEW: Save BLP outputs (+ join R2_tau) -----------------------
if (length(diag_blp_rows)) {
  ...
  data.table::fwrite(blp_out, "cf_M2_V7_up_BLP_per_covariate.csv")
  cat("\n[Saved]\n  - cf_M2_V7_up_BLP_per_covariate.csv\n", sep = "")
}

# ----------------------- Example subgroup readout -----------------------
keys <- dt[, .(row_id, token_id, log_buyern_total_value)]
if (file.exists("cf_M2_V7_up_results_CATEs.csv")) {
  sub <- merge(fread("cf_M2_V7_up_results_CATEs.csv"), keys, by = "row_id", all.x = TRUE)
  sub[, buyer_val_q := cut(log_buyern_total_value,
                           breaks = quantile(log_buyern_total_value, probs = seq(0,1,0.2), na.rm=TRUE),
                           include.lowest = TRUE)]
  print(sub[ , .(
    n = .N,
    mean_pct = mean(cate_pct),
    sd_pct   = sd(cate_pct)
  ), by = .(treatment, buyer_val_q)][order(treatment, buyer_val_q)])
}
